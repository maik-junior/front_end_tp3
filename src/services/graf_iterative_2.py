...
def iteratividade_graf():   
    ...
    selected_df = st.multiselect(
        ...
        options=['Rio_de_Janeiro','Sao_Paulo','Porto_alegre'],key='6')
    ...
    #==> Definindo ordenacao
    ordenar = st.radio('Ordenar os dados:', ('Ordem crescente', 'Ordem decrescente'),key='4')
    ...
    if selected_df:
        for tabela in selected_df:
            ...
            if df is not None:
                ...
                coluna_para_ordenar = st.selectbox(f'Selecione a coluna para ordenar em {tabela}:', df.columns, key='7')
